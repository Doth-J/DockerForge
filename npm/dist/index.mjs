...
function writeMakefile(name, language) {
...
function writeDockerfiles(docker) {
    ...
    fs.writeFileSync(path.join(process.cwd(), "docker-compose.yaml"), docker.compose_dev);
    fs.writeFileSync(path.join(process.cwd(), "docker-compose.production.yaml"), docker.compose_prod);
    if (docker.nodemon)
...
function main() {
    ...
    program
        ...
        .description(`🐳 ${kleur.cyan("Dockerfile generator cli tool for CD/CI operations.")}`)
        ...
        .option("-s,--scaffold", kleur.blue("Scaffold template project with 'src' structure"))
        .addOption(new Option("-c,--config <string>", kleur.blue("Create project config file")).choices(["json", "toml", "yaml", "ini", "none"]))
        .addOption(new Option("-l,--language <string>", kleur.blue("Target project language")).choices(["typescript", "javascript", "python", "golang", "rust"]))
        .addHelpOption(new Option("-h,--help", kleur.blue("Display this help message")))
        .version(`dockerforge v.1.0.0`, "-v, --version", kleur.blue("Output the current version"))
        ...
        const response = yield prompts([
            ...
            {
                type: () => options.scaffold || fs.existsSync(path.join(process.cwd(), "src"))
                    ? null
                    : "confirm",
                name: "scaffold",
        ...
        const name = options.name || response.name, config = options.config || response.config, language = options.language || response.language, scaffold = (options.scaffold &&
        ...
        if (config && config != "none") {
            ...
            if (fs.existsSync(path.join(process.cwd(), "config." + config))) {
                progress.fail(`${kleur.bold(config.toUpperCase())} config file already exists!`);
            }
            else {
                writeConfig(config);
                progress.stopAndPersist({
        ...
        switch (language) {
            case "typescript": {
                ...
                writeDockerfiles(Object.assign({}, typescript.dockerize(name.toLowerCase(), config != "none" ? config : "")));
                progress.stopAndPersist({
                    symbol: "🐳",
                ...
                if (scaffold) {
                    ...
                    fs.writeFileSync(path.join(process.cwd(), "src", "index.ts"), index);
            ...
            case "javascript": {
                progress.start("Creating dockerfiles...");
                writeDockerfiles(Object.assign({}, javascript.dockerize(name.toLowerCase(), config != "none" ? config : "")));
                progress.stopAndPersist({
                    symbol: "🐳",
                    text: `Created ${kleur.bold(language)} dockerfiles!`,
                });
                if (scaffold) {
                    ...
                    fs.writeFileSync(path.join(process.cwd(), "src", "index.js"), index);
                    progress.text = "Creating package.json...";
                    fs.writeFileSync(path.join(process.cwd(), "package.json"), pkg);
                    progress.stopAndPersist({
                        symbol: "📒",
            ...
            case "python": {
                progress.start("Creating dockerfiles...");
                writeDockerfiles(Object.assign({}, python.dockerize(name.toLowerCase(), config != "none" ? config : "")));
                progress.stopAndPersist({
                    symbol: "🐳",
                    text: `Created ${kleur.bold(language)} dockerfiles!`,
                });
                if (scaffold) {
                    ...
                    fs.writeFileSync(path.join(process.cwd(), "src", "index.py"), index);
            ...
            case "golang": {
                progress.start("Creating dockerfiles...");
                writeDockerfiles(Object.assign({}, go.dockerize(name.toLowerCase(), config != "none" ? config : "")));
                progress.stopAndPersist({
                    symbol: "🐳",
                    text: `Created ${kleur.bold(language)} dockerfiles!`,
                });
            ...
            case "rust": {
                progress.start("Creating dockerfiles...");
                writeDockerfiles(Object.assign({}, rust.dockerize(name.toLowerCase(), config != "none" ? config : "")));
                progress.stopAndPersist({
                    symbol: "🐳",
                    text: `Created ${kleur.bold(language)} dockerfiles!`,
                });
